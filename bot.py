...
async def cb_quality_download(cb: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    label = QUALITY_LABELS.get(quality, quality)
    await cb.message.edit_text(t(lang, "downloading", q=label), parse_mode="Markdown")
    result = await download_video(url, quality)
    if not result.success:
        err_key = "error_size" if result.error == "size" else "error_download"
        await cb.message.edit_text(t(lang, err_key))
    ...
    try:
        if result.is_audio:
            with open(result.path, "rb") as f:
                await cb.message.answer_audio(f, caption=t(lang, "done"), parse_mode="Markdown")
        else:
            with open(result.path, "rb") as f:
                await cb.message.answer_video(f, caption=t(lang, "done"), parse_mode="Markdown")
        await cb.message.delete()
        db.log_download(uid, url, quality, "ok", result.file_size)
    except Exception as e:
        log.error("Send error: %s", e)
        await cb.message.edit_text(t(lang, "error_size"))
        db.log_download(uid, url, quality, "error")
    finally:
        if result.path:
            try:
                import os as _os
                _os.remove(result.path)
            except Exception:
                pass
    await cb.answer()
